...
def medusa_forward(input_ids, model, tokenizer, medusa_choices, temperature, posterior_threshold, posterior_alpha, max_steps = 512):
    ...
    if hasattr(model, "past_key_values"):
        ...
        (
            ...
            current_length_data,
        ) = initialize_past_key_values(model.base_model, medusa_num_decoder_layers = model.medusa_num_decoder_layers)
    ...
    medusa_logits, logits = initialize_medusa_austin(
    ...
    for idx in range(max_steps): 
        ...
        hidden_states, logits, outputs = tree_decoding_austin(
                model,
                tree_candidates,
                past_key_values,
        ...
        input_ids, logits, medusa_logits, new_token = update_inference_inputs_austin(
                model,
                input_ids,
                candidates,
                best_candidate,
        ...
        if tokenizer.eos_token_id in input_ids[0, input_len:].tolist():
            eos_token_position = input_ids[0, input_len:].tolist().index(tokenizer.eos_token_id)
            new_token = eos_token_position + 1
            break
        if new_token > 1024:
            break
