...
class CustomizedTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if hasattr(model, "module"):
            embed_tokens = model.module.base_model.model.embed_tokens
            medusa_gram = model.module.medusa_gram_head
        else:
            embed_tokens = model.base_model.model.embed_tokens
            medusa_gram = model.medusa_gram_head
        # Compute loss
        embed = embed_tokens(inputs["input_ids"])
        logits = medusa_gram(embed)
        # logits = model(
        ...
        medusa_logits = logits[:, : -1].contiguous()
        medusa_labels = labels[..., 1:].contiguous()
        medusa_logits = medusa_logits.view(-1, logits.shape[-1])
        medusa_labels = medusa_labels.view(-1)
        medusa_labels = medusa_labels.to(medusa_logits.device)
        loss = loss_fct(medusa_logits, medusa_labels)
        ...
        for k in range(1, 6):
            ...
            log[f"medusa_gram_top{k}"] = correct.float().mean().item()
        log[f"medusa_gram_loss"] = loss.item()
        self.log(log)
...
def train():
    ...
    if hasattr(medusa_lm_head, "module"):
        medusa_gram_head = medusa_lm_head.module.medusa_gram_head
    else:
        medusa_gram_head = medusa_lm_head.medusa_gram_head

    torch.save(
        medusa_gram_head.state_dict(),
        os.path.join(training_args.output_dir, "medusa_gram_head.pt"),
    )
